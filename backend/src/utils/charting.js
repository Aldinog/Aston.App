const { fetchProfile } = require('./yahoofinance');
const { getPersistentCandles } = require('./persistence');
...
async function getChartData(symbol, interval = '1d', limit = 300) {
    ...
    // 1. Fetch Primary Data (Persistent)
    const candles = await getPersistentCandles(symbol, interval, limit);
    ...
    // 2. Fetch Daily Trend Data (if current interval is intraday/smaller than daily)
    let dailyTrend = 'neutral';
    const intradayIntervals = ['15m', '60m', '1h', '4h'];
    if (intradayIntervals.includes(interval)) {
        const dCandles = await getPersistentCandles(symbol, '1d', 50);
