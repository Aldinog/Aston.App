    ...
    try {
        ...
        for (const { symbol } of targetSymbols) {
            try {
                ...
                const { candles, markers, levels } = chartData;
                if (!candles || candles.length < 30) continue;

                // 5. Detect Signal based on H1 Markers (matches Smart Chart UI)
                let signal = { action: 'WAIT', reason: null };

                // Get the most recent marker
                const latestMarker = markers && markers.length > 0 ? markers[markers.length - 1] : null;
                const lastCandle = candles[candles.length - 1];
                const prevCandle = candles[candles.length - 2];

                // Check if marker timestamp matches current or previous H1 bar
                if (latestMarker && (latestMarker.time === lastCandle.time || (prevCandle && latestMarker.time === prevCandle.time))) {
                    // Signal found in recent H1 bars!
                    // To get the 'reason', we find the candle index and run detection
                    const markerIndex = candles.findIndex(c => c.time === latestMarker.time);
                    if (markerIndex !== -1) {
                        const indicators = computeIndicators(candles);
                        signal = detectAdvancedSignal(candles, indicators.all, markerIndex, levels);
                    }
                }
                ...
                await supabase.from('scan_state').upsert({
                    ...
                    timeframe: 'H1' // Explicitly track timeframe
                });
