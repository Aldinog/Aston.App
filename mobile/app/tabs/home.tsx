...
export default function Home() {
    ...
    const fetchUserProfile = async () => {
        try {
            ...
            if (session?.user) {
                // FORCE Select from public.users to get latest username
                ...
                if (profile) {
                    ...
                    // Fallback to session metadata if public profile missing (shouldn't happen with fix)
    ...
    // Pro Check Logic: Status is 'pro' AND (Expiry is null OR Expiry is in future)
    // Adjust logic based on your strictness preference. If expiry is mandatory, remove null check.
    const isPro = user?.membership_status === 'pro' && (!user.pro_expires_at || new Date(user.pro_expires_at) > new Date());
                    ...
                    <View className="flex-row items-center">
                        ...
                        <View>
                            ...
                            <Text className="text-foreground font-black text-xl tracking-tight capitalize">
                                @{user?.username || user?.full_name?.split(' ')[0] || user?.email?.split('@')[0] || 'Trader'}
                            </Text>
